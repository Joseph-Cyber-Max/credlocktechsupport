...
import { useSupportAuth, signOutUser } from '../lib/auth'
...
function AdminCenter() {
  const { user, loading: authLoading } = useSupportAuth()
  ...
  const isAdmin = String(user?.Role || profile?.Role || '').toUpperCase() === 'ADMIN'
  const load = async () => {
  ...
  const filtered = useMemo(() => { const term=query.trim().toLowerCase(); return term ? records.filter(r=>Object.values(r).some(v=>String(v??'').toLowerCase().includes(term))) : records }, [records,query])
