...
function AdminCenter() {
  ...
  const [source, setSource] = useState<DataSource>('FIREBASE')
  const active = tabs.find(item => item.id === tab) || tabs[0]

  const load = async () => {
    if (!user) return
    setLoading(true); setError('')
    try { setRecords(await listPrimary(active.collection, 1000)); setSource('FIREBASE') }
    catch (err) { setError(err instanceof Error ? err.message : 'Unable to load administration data.') }
    finally { setLoading(false) }
  }

  useEffect(() => { void load() }, [active.collection, user?.uid])
  ...
  if (authLoading) return <div className="admin-page"><main className="admin-content"><section className="admin-hero"><div><p>System administration</p><h1>Loading control center…</h1></div></section></main></div>
  ...
  return <div className="admin-page">
    <header className="admin-topbar"><div className="admin-brand"><div className="admin-mark"><ServerCog size={20}/></div><div><span>Credlock</span><strong>Admin Control Center</strong></div></div><div className="admin-top-actions"><span className="admin-source primary"><Database size={14}/> {source === 'FIREBASE' ? 'Firestore primary' : 'Sheets fallback'}</span><span className="admin-user">{user.email || user.uid}</span><button className="admin-back" onClick={() => void signOutUser()}><LogOut size={15}/> Sign out</button><Link to="/" className="admin-back"><ArrowLeft size={15}/> Workspace</Link></div></header>
    <main className="admin-content">
      <section className="admin-hero"><div><p>System administration</p><h1>Control access, people and platform settings.</h1><span>Firebase Authentication + Cloud Firestore are now the primary control plane.</span></div><div className="admin-status"><Activity size={16}/><div><strong>Control plane</strong><small>Authenticated Firestore session</small></div></div></section>
