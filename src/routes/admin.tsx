...
function AdminCenter() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      try {
        const next = await getPrimary('Users', user.uid)
  ...
  useEffect(() => { if (isAdmin) void load() }, [active.collection, user?.uid, isAdmin])
  ...
  return <div className="admin-page">
    ...
    <main className="admin-content">
      <section className="admin-hero"><div><p>System administration</p><h1>Control access, people and platform settings.</h1><span>Firebase Authentication + Cloud Firestore are now the primary control plane.</span></div><div className="admin-status"><Activity size={16}/><div><strong>Control plane</strong><small>Authenticated administrator session</small></div></div></section>
